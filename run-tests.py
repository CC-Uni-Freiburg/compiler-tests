...
enable_tracing()
results = [run_tests("var", compiler, "var", None, interp_Pvar.InterpPvar().interp_P, type_check_Cif.TypeCheckCif().type_check, interp_Cif.InterpCif().interp)]
if hasattr(compiler, "allocate_registers"):
    results.append(run_tests("regalloc", compiler, "regalloc", None, interp_Pvar.InterpPvar().interp_P, type_check_Cif.TypeCheckCif().type_check, interp_Cif.InterpCif().interp))
